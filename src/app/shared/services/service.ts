...
export class ApiService {
  ...
  getColumns(entity_code: String, isAdmin: Boolean = false) {
    if (isAdmin) {
      return _.find(this.getSessionValues('admin_enities'), x => x.entity_code === entity_code).entity_schema_access;
    }

    return _.find(this.getSessionValues('accessible_entities'), x => x.entity_code === entity_code).entity_schema_access;

  }
  getAccessCode(entity_code: String, isAdmin: Boolean = false) {
    if (isAdmin) {
      return _.find(this.getSessionValues('admin_enities'), x => x.entity_code === entity_code).entity_access_code;
    }
    return _.find(this.getSessionValues('accessible_entities'), x => x.entity_code === entity_code).entity_access_code;

  }
  getEntityLabel(entity_code: String, isAdmin: Boolean = false) {
    if (isAdmin) {
      return _.find(this.getSessionValues('admin_enities'), x => x.entity_code === entity_code).label;
    }
